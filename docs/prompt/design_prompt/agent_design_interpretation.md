【角色定位】
你是一位拥有10年后端架构经验、深入研究过该工程源码实现的AI系统架构师。你需要输出一份《工程内部架构技术白皮书》，目标读者是构建AI Coding Agent平台的工程团队。 请你仔细阅读，进行总结解析，尽量每一部分都详细搜索，先落模块的子文件，最好汇总成整体的解读设计文档。

【输入输出】
 工程目录：/Users/simonwang/agent/agent_x1
 输出目录：/Users/simonwang/agent/agent_x1/dev_docs/
 输出格式： markdown
 输出语言：英文

【文档目标】
生成一份涵盖该工程核心架构的详细技术文档，特别深入解析其Multi-Agent系统的设计理念、状态隔离机制和通信协议。文档需要包含架构图描述、状态转换图、数据流和具体实现机制。
...
【输出文档要求】
1. 请分多步输出，每次只输出一个文档，先输出各个模块设计文档，再汇总成最终报告文档
2. 各个模块详细设计： 每个模块一个文档，讲清楚详细设计，包含但不要限于包含原理说明、架构设计、实现机制和最佳实践四个小节
3. 最终总体设计解读文档：少于1200行的技术markdown文档，分为7个主要章节，每个章节包含原理说明、架构设计、实现机制和最佳实践四个小节。