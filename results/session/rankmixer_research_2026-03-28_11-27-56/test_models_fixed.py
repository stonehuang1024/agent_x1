...
def test_model(model, model_name, batch_size=4, input_dim=624):
    """Test a single model"""
    print(f"\nTesting {model_name}...")
    ...
    try:
        ...
        if isinstance(output, tuple):
            ...
            print(f"  Output shape: {output.shape}, Aux loss: {aux_loss.item() if aux_loss is not None else None}")
        else:
...
def main():
    ...
    input_dim = 624  # 39 features * 16 dims
    results = {}
    ...
    rankmixer_models = {
        ...
        'RankMixer-MoE': create_rankmixer_moe(input_dim),
    }
    
    for name, model in rankmixer_models.items():
        results[name] = test_model(model, name, input_dim=input_dim)
