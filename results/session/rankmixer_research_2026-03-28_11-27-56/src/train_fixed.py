...
def train_epoch(model: nn.Module, train_loader: DataLoader, optimizer: optim.Optimizer,
                criterion: nn.Module, device: torch.device, use_moe: bool = False,
                input_dim: int = 624) -> Dict[str, float]:
    """Train for one epoch"""
    model.train()
    total_loss = 0.0
    all_predictions = []
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if use_moe:
            ...
            loss = criterion(predictions, labels) + (aux_loss if aux_loss is not None else 0)
        else:
...
def evaluate(model: nn.Module, data_loader: DataLoader, criterion: nn.Module,
             device: torch.device, use_moe: bool = False, input_dim: int = 624) -> Dict[str, float]:
    """Evaluate model"""
    model.eval()
    total_loss = 0.0
    all_predictions = []
...
def run_experiment(config: Dict) -> List[Dict]:
    ...
    input_dim = 624  # 39 features * 16 dims (standard for Criteo-like data)
